...
async function generateCard() {
	...
	const materials = document.querySelector('#materials').value;
	const constraint = document.querySelector('#constraint').value;
	const draft = draftInput.value.trim() || example;

	// Try to call local AI endpoint; if it fails, fallback to local generation
	try {
		const resp = await fetch('/api/generate', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ draft, audience, subject, format, deadline, materials, constraint }),
		});
		if (resp.ok) {
			const json = await resp.json();
			document.querySelector('#taskTitle').value = json.title || `${format} по предмету «${subject}»`;
			document.querySelector('#taskContext').value = json.context || `${draft}\n\nЦелевая аудитория: ${audience}. Уже есть: ${materials}.`;
			document.querySelector('#taskResult').value = json.result || `Готовый ${format.toLowerCase()} для ${audience.toLowerCase()} со структурой занятия, заданиями и способом проверить понимание темы. Срок: ${deadline}.`;
			document.querySelector('#taskCriteria').value = json.criteria || `Понятная структура урока\nМинимум 3 практических задания\nУчтено ограничение: ${constraint}`;
			showToast('Карточка сгенерирована AI');
			return;
		}
	} catch (e) {
		console.warn('AI call failed, using local generator', e);
	}

	// Fallback local generation
	document.querySelector('#taskTitle').value = `${format} по предмету «${subject}»`;
	document.querySelector('#taskContext').value = `${draft}\n\nЦелевая аудитория: ${audience}. Уже есть: ${materials}.`;
	document.querySelector('#taskResult').value = `Готовый ${format.toLowerCase()} для ${audience.toLowerCase()} со структурой занятия, заданиями и способом проверить понимание темы. Срок: ${deadline}.`;
